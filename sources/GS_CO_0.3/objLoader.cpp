...
unsigned int objloader::loadTexture(const char* filename)
{
	...
	AUX_RGBImageRec *textura = new AUX_RGBImageRec(filename);
	

	glBindTexture(GL_TEXTURE_2D, num);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, textura->sizeX, textura->sizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, textura->data);
